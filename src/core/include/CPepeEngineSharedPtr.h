...
public:
    ...
    CPepeEngineSharedPtr(T* t) : ptr(t) {
        if (ptr) {
            ptr->ref();
        }
    }
    CPepeEngineSharedPtr(const CPepeEngineSharedPtr& p) : ptr(p.ptr) {
        if (ptr) {
            ptr->ref();
        }
    }
    virtual ~CPepeEngineSharedPtr() {
        if (ptr) {
            ptr->deref();
        }
    }
    ...
    bool operator==(const CPepeEngineSharedPtr<T>& p) const {
        return (ptr == p.ptr);
    }
    bool operator!=(const CPepeEngineSharedPtr<T>& p) const {
        return (ptr != p.ptr);
