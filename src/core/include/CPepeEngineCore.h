...
public:	
	...
	CPepeEngineSceneManager* getSceneManager() const {return m_pSceneManager;}
	IPepeEngineRenderer* getRenderer() const {return m_pRenderer;}
	IPepeEngineWindow* getWindow() const {return m_pWindow;}
	CPepeEngineConfig* getConfig() const {return m_pConfig;}
	IPepeEngineTimer* getTimer() const {return m_pTimer;}
	 ...
	 */
	void setFrameSmoothingPeriod(float fPeriod) {m_fFrameSmoothingTime = fPeriod;}
	
	/**
	 *	Gets the period over which PepeEngine smooths out fluctuations in frame times. 
	 */
	float getFrameSmoothingPeriod(void) const {return m_fFrameSmoothingTime;}

	void run();

	void addRenderer(IPepeEngineRenderer* pRenderer);
	RendererList* getAvailableRenderers() const {return m_pRendererList;}
